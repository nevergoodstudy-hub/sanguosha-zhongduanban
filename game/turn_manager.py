...
class TurnManager:
    ...
    def _execute_draw_phase(self, player: Player) -> None:
        ...
        # 基础摸牌数
        draw_count = 2
        if self.engine.skill_system and player.has_skill(SkillId.ZISHOU):
            self.engine.skill_system.trigger_skill(SkillId.ZISHOU, player, self.engine, mode="draw")
        ...
        # 突袭技能在摸牌阶段触发
        if player.has_skill(SkillId.TUXI) and self._try_trigger_tuxi(player, draw_count):
            return  # 突袭成功，跳过正常摸牌

        cards = self.engine.deck.draw(draw_count)
        player.draw_cards(cards)
        self.engine.notify_cards_obtained(player, cards, source=player, reason="draw_phase")
            ...
            if tuxi_handler:
                ...
                if result:
                    ...
                    if remaining > 0:
                        ...
                        player.draw_cards(cards)
                        self.engine.notify_cards_obtained(
                            player,
                            cards,
                            source=player,
                            reason="draw_phase",
