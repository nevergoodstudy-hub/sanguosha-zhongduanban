...
class CardResolver:
    ...
    def use_jiu(self, player: Player, card: Card) -> bool:
        ...
        if player.is_dying:
            player.heal(1)
            ctx.log_event(
                "use_card", _t("resolver.jiu_heal", name=player.name), source=player, card=card
            )
            ctx.deck.discard([card])
            return True
        ...
        if player.use_alcohol():
            ctx.log_event(
                "use_card", _t("resolver.use_jiu", name=player.name), source=player, card=card
            )
            ctx.deck.discard([card])
            return True
    ...
    def use_nanman(self, player: Player, card: Card) -> bool:
        ...
        for target in ctx.get_other_players(player):
            if ctx._request_wuxie(card, player, target):
                ctx.log_event(
                    "effect", _t("resolver.nullified_for", card=_t("card.nanman"), name=target.name)
                )
                continue
            ...
            if sha_count < 1:
                ...
                ctx.log_event("effect", _t("resolver.played_sha", name=target.name))
    ...
    def use_wanjian(self, player: Player, card: Card) -> bool:
        ...
        for target in ctx.get_other_players(player):
            ...
            if shan_count < 1:
                ...
                ctx.log_event("effect", _t("resolver.played_shan", name=target.name))
    ...
    def use_taoyuan(self, player: Player, card: Card) -> bool:
        ...
        for i in range(len(ctx.players)):
            ...
            if ctx._request_wuxie(card, player, p):
                ctx.log_event(
                    "effect", _t("resolver.nullified_for", card=_t("card.taoyuan"), name=p.name)
                )
                continue
    ...
    def use_shunshou(self, player: Player, card: Card, targets: list[Player]) -> bool:
        ...
        if ctx.calculate_distance(player, target) > 1:
            ctx.log_event("error", _t("resolver.shunshou_too_far", name=target.name))
            player.draw_cards([card])
            return False
        ...
        if stolen_card:
            ctx.log_event(
                "effect", _t("resolver.card_stolen", player=player.name, target=target.name)
            )
    ...
    def use_jiedao(self, player: Player, card: Card, targets: list[Player]) -> bool:
        ...
        ctx.log_event(
            ...
            _t(
                "resolver.use_jiedao",
                player=player.name,
                wielder=wielder.name,
                target=sha_target.name,
            ),
            source=player,
            target=wielder,
            card=card,
        ...
        if sha_count >= 1:
            ...
            if shan_count >= 1:
                ctx.log_event("dodge", _t("combat.dodge_success", name=sha_target.name))
            else:
    ...
    def use_huogong(self, player: Player, card: Card, targets: list[Player]) -> bool:
        ...
        ctx.log_event(
            "effect", _t("resolver.shown_card", name=target.name, card=shown_card.display_name)
        )
        ...
        if discard_card:
            ...
            ctx.log_event(
                "effect",
                _t(
                    "resolver.discard_for_huogong", name=player.name, card=discard_card.display_name
                ),
            )
            ctx.deal_damage(player, target, 1, damage_type="fire")
        else:
            ctx.log_event("effect", _t("resolver.huogong_no_discard", name=player.name))
    ...
    def use_lebusishu(self, player: Player, card: Card, targets: list[Player]) -> bool:
        ...
        ctx.log_event(
            "effect", _t("resolver.delay_placed", card=_t("card.lebusishu"), name=target.name)
        )
        return True
    ...
    def use_bingliang(self, player: Player, card: Card, targets: list[Player]) -> bool:
        ...
        if distance > 1:
            ctx.log_event(
                "error", _t("resolver.bingliang_too_far", name=target.name, distance=distance)
            )
            player.draw_cards([card])
            return False
        ...
        target.judge_area.insert(0, card)
        ctx.log_event(
            "effect", _t("resolver.delay_placed", card=_t("card.bingliang"), name=target.name)
        )
        return True
    ...
    def use_shandian(self, player: Player, card: Card, targets: list[Player] = None) -> bool:
        ...
        for c in player.judge_area:
            if c.name == CardName.SHANDIAN:
                ctx.log_event(
                    "error", _t("resolver.delay_exists", name=player.name, card=_t("card.shandian"))
                )
                player.draw_cards([card])
                return False

        ctx.log_event(
            "use_card", _t("resolver.use_shandian", name=player.name), source=player, card=card
        )
        player.judge_area.insert(0, card)
        ctx.log_event(
            "effect", _t("resolver.delay_placed", card=_t("card.shandian"), name=player.name)
        )
        return True
    ...
    def use_tiesuo(self, player: Player, card: Card, targets: list[Player] | None = None) -> bool:
        ...
        if not targets:
            ctx.log_event(
                "use_card", _t("resolver.tiesuo_recast", name=player.name), source=player, card=card
            )
            ctx.deck.discard([card])
        ...
        for target in targets[:2]:
            ...
            ctx.log_event(
                "effect",
                _t(
                    "resolver.chain_status",
                    name=target.name,
                    status=status,
                    chained=target.is_chained,
                ),
            )
