...
def handle_jianxiong(player: Player, engine: GameEngine,
                     damage_card: Card | None = None, **kwargs) -> bool:
    """奸雄：受到伤害后，可以获得造成伤害的牌"""
    if damage_card:
        if damage_card in engine.deck.discard_pile:
            engine.deck.discard_pile.remove(damage_card)
            player.draw_cards([damage_card])
            engine.log_event("skill", _t("skill_msg.jianxiong", name=player.name, card=damage_card.display_name))
            return True
    return False
...
def handle_fankui(player: Player, engine: GameEngine,
...
def handle_guicai(player: Player, engine: GameEngine,
...
def handle_ganglie(player: Player, engine: GameEngine,
                   source: Player = None, **kwargs) -> bool:
    ...
    engine.log_event("skill", _t("skill_msg.ganglie", name=player.name, card=judge_card.display_name))
...
@skill_handler("tuxi")
def handle_tuxi(player: Player, engine: GameEngine,
...
def handle_duanliang(player: Player, engine: GameEngine,
    ...
    if any(c.name == CardName.BINGLIANG for c in target.judge_area):
        engine.log_event("error", _t("resolver.delay_exists", name=target.name, card=_t("card.bingliang")))
        return False
...
def handle_shensu(player: Player, engine: GameEngine,
    ...
    engine.log_event("skill",
        _t("skill_msg.shensu", name=player.name, choice=choice, target=target.name))
